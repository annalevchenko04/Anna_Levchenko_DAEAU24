...
INSERT INTO public.film (title, description, release_year, language_id, original_language_id, rental_duration, rental_rate, length, replacement_cost, rating, special_features, last_update)
SELECT movie.title, movie.description, movie.release_year, lang.language_id, lang.language_id, movie.rental_duration, movie.rental_rate, movie.length, movie.replacement_cost, movie.rating::public.mpaa_rating, movie.special_features::text[], CURRENT_DATE
FROM (VALUES 
    ('Coco', 'A story of a young boy who wants to be a musician...', 2010, 7, 4.99, 148, 19.99, 'PG-13'::public.mpaa_rating, ARRAY['Trailers']::text[]),
    ('Spotlight', 'Sacha Pfeiffer interview victims and try to unseal sensitive documents.', 2001, 14, 9.99, 142, 19.99, 'R'::public.mpaa_rating, ARRAY['Commentaries']::text[]),
    ('Little Women', 'Amy has a chance encounter with Theodore...', 2019, 21, 19.99, 175, 19.99, 'R'::public.mpaa_rating, ARRAY['Deleted Scenes']::text[])
) AS movie (title, description, release_year, rental_duration, rental_rate, length, replacement_cost, rating, special_features)
JOIN public.language lang ON LOWER(lang.name) = LOWER('English')
WHERE NOT EXISTS (SELECT 1 FROM public.film f WHERE LOWER(f.title) = LOWER(movie.title))
RETURNING film_id;
...
-- Insert actors into the public.actor table using a single insert
...
-- Map actors to films in the public.film_actor table
...
JOIN public.inventory i ON ir.inventory_id = i.inventory_id
