SELECT 
    ...
    STRING_AGG(DISTINCT f.title, ', ') AS horror_movies,  -- Aggregate horror movie titles into one string
...
WHERE UPPER(ca.name) = UPPER('HORROR')  -- Filter for horror category, using UPPER for case-insensitivity
